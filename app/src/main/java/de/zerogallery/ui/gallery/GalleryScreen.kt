 ...
 * grouping - specifically [MediaGroupingMode.FOLDER] - reorders items into sections, so the flat,
 * on-screen display order no longer matches [GalleryUiState.Content.items]' original chronological
 * order. [MediaDetailScreen] needs that *same displayed order* (not the original one) so that
 * swiping through it matches whatever the grid was just showing when a tile was tapped - hence
 * [displayedItems] is computed once here and threaded through to both [MediaGrid] and
 * [MediaDetailScreen].
 */
@Composable
fun GalleryRoute(viewModel: GalleryViewModel) {
    ...
    val displayedItems = remember(groups) { groups.flatMap { it.items } }
 ...
 * what tapping it will switch *away* from, and section headers appearing/disappearing in the grid
 * below give immediate, self-explanatory feedback for the change - no separate confirmation toast
 * needed.
 */
